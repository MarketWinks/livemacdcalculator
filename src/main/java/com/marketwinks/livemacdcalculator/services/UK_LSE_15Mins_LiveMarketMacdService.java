...
import com.marketwinks.livemacdcalculator.model.uk_lse_15mins_livemarketmacd;
...
@RestController
@RequestMapping("/uk_lse_15mins_livemarketmacd")
public class UK_LSE_15Mins_LiveMarketMacdService {

	@Autowired
	private UK_LSE_15Mins_LiveMarketMacdRepository UK_LSE_15Mins_LiveMarketMacdRepository;

	@Autowired
	private UK_LSE_15Mins_LiveMarketMacdjsonRepository UK_LSE_15Mins_LiveMarketMacdjsonRepository;
	...
	public boolean UK_LSE_15Mins_LiveMarketMacdParser(@PathVariable String symbol) {
		System.gc();
...
//		
		...
		List<uk_lse_15mins_livemarketmacd> MarketFeeds_full = mongoTemplate.find(query, uk_lse_15mins_livemarketmacd.class);
		...
		try {
			...
			List<uk_lse_15mins_livemarketmacd> MarketFeeds = new ArrayList<uk_lse_15mins_livemarketmacd>();
			...
			for (int i = 0; i < MarketFeedsSizeForSymbol; i++) {
				...
				Collections.sort(MarketFeeds, new SortbyLatestTime_15min());
				...
				for (int index = 0; index < MarketFeedsSizeForSymbol; index++) {
					if (MarketFeeds.get(index).getEma12().toString().equals("calculating")
						...
						uk_lse_15mins_livemarketmacd MarketFeed = new uk_lse_15mins_livemarketmacd();
						...
						uk_lse_15mins_livemarketmacd result = UK_LSE_15Mins_LiveMarketMacdRepository.save(MarketFeed);
						...
						UK_LSE_15Mins_LiveMarketMacdRepository.deleteById(MarketFeeds.get(calcStartindex).get_id());
			...
			List<uk_lse_15mins_livemarketmacd> MarketFeedsForMacdJson = MarketFeeds_full.stream()
					.filter(a -> a.getSymbol().equals(symbol)).collect(Collectors.toList());

			Collections.sort(MarketFeedsForMacdJson, new SortbyLatestTime_15min());
			...
			uk_lse_15mins_livemarketmacdjson uk_lse_15mins_macdjson = new uk_lse_15mins_livemarketmacdjson();
			uk_lse_15mins_macdjson.setMacdjsonref("uk_lse_15mins_macdjson_" + symbol);
			uk_lse_15mins_macdjson.setMacdjson(macdDataforSaving);
			...
			org.bson.Document doc = uk_lse_15mins_livemarketmacdjsonCollection
					.find(new org.bson.Document("macdjsonref", "uk_lse_15mins_macdjson_" + symbol)).first();
			...
			UK_LSE_15Mins_LiveMarketMacdjsonRepository.deleteById(idofdocext);
			// } else {
...
//			mongoClient.close();
	...
	@RequestMapping(value = "/UK_LSE_15Mins_LiveMarketMacdUpdateByObjectId/update/{objectId}/{ema12}/{ema26}/{signal}/{macd}/{histogram}", method = RequestMethod.GET)
	public boolean LiveMarketMacdForAnyTimeParser(@PathVariable String objectId, @PathVariable String ema12,
		...
		for (int u = 0; u < MarketFeeds_full.size(); u++) {
			...
			if (MarketFeeds_full.get(u).get_id().equals(objectId)) {
				...
				// attempt to delete
				UK_LSE_15Mins_LiveMarketMacdRepository.deleteById(objectId);
				System.out.println("Duplicate record delete attempted");
...
class SortbyLatestTime_15min implements Comparator<uk_lse_15mins_livemarketmacd> {
	// Used for sorting in ascending order of
	// roll number
	public int compare(uk_lse_15mins_livemarketmacd a, uk_lse_15mins_livemarketmacd b) {
