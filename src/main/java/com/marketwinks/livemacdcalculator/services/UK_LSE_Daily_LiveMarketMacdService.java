...
@RestController
@RequestMapping("/uk_lse_daily_livemarketmacd")
public class UK_LSE_Daily_LiveMarketMacdService {
	...
	public boolean UK_LSE_Daily_LiveMarketMacdParser(@PathVariable String symbol) {
		System.gc();
		...
		try {
			...
			// List<uk_lse_daily_livemarketmacd> MarketFeeds_full =
			// UK_LSE_Daily_LiveMarketMacdRepository.findAll();

			List<uk_lse_daily_livemarketmacd> MarketFeeds = new ArrayList<uk_lse_daily_livemarketmacd>();
			...
			for (int i = 0; i < MarketFeedsSizeForSymbol; i++) {
				...
				// MarketFeeds_full = UK_LSE_Daily_LiveMarketMacdRepository.findAll();
				MarketFeeds.clear();
				...
				Collections.sort(MarketFeeds, new SortbyLatestTime_daily());
				...
				for (int index = 0; index < MarketFeedsSizeForSymbol; index++) {
					if (MarketFeeds.get(index).getEma12().toString().equals("calculating")
						...
						uk_lse_daily_livemarketmacd MarketFeed = new uk_lse_daily_livemarketmacd();
						...
						uk_lse_daily_livemarketmacd result = UK_LSE_Daily_LiveMarketMacdRepository.save(MarketFeed);
						...
						UK_LSE_Daily_LiveMarketMacdRepository.deleteById(MarketFeeds.get(calcStartindex).get_id());
			...
			List<uk_lse_daily_livemarketmacd> MarketFeedsForMacdJson = MarketFeeds_full.stream()
					.filter(a -> a.getSymbol().equals(symbol)).collect(Collectors.toList());

			Collections.sort(MarketFeedsForMacdJson, new SortbyLatestTime_daily());
			...
			uk_lse_daily_livemarketmacdjson uk_lse_daily_macdjson = new uk_lse_daily_livemarketmacdjson();
			uk_lse_daily_macdjson.setMacdjsonref("uk_lse_daily_macdjson_" + symbol);
			uk_lse_daily_macdjson.setMacdjson(macdDataforSaving);
			...
			UK_LSE_Daily_LiveMarketMacdjsonRepository.deleteById(idofdocext);
			// } else {
			...
			uk_lse_daily_livemarketmacdjson jsonsaveresult = UK_LSE_Daily_LiveMarketMacdjsonRepository
					.save(uk_lse_daily_macdjson);
			// uk_lse_daily_macdjson_<symbol> --> macdDataforSaving
			//mongoClient.close();
	...
	// "/UK_LSE_Daily_LiveMarketMacdForAnyTime/update/{symbol}/{price}/{ema12}/{ema26}/{signal}/{macd}/{histogram}",
	...
	// MarketFeed.setHistogram(histogram);
	//
	// uk_lse_daily_livemarketmacd result =
	// UK_LSE_Daily_LiveMarketMacdRepository.save(MarketFeed);
	// execution_result = true;
	// return execution_result;
	//
	// }

	@RequestMapping(value = "/UK_LSE_Daily_LiveMarketMacdUpdateByObjectId/update/{objectId}/{ema12}/{ema26}/{signal}/{macd}/{histogram}", method = RequestMethod.GET)
	public boolean LiveMarketMacdForAnyTimeParser(@PathVariable String objectId, @PathVariable String ema12,
		...
		for (int u = 0; u < MarketFeeds_full.size(); u++) {
			...
			if (MarketFeeds_full.get(u).get_id().equals(objectId)) {
				...
				// attempt to delete
				UK_LSE_Daily_LiveMarketMacdRepository.deleteById(objectId);
				System.out.println("Duplicate record delete attempted");
