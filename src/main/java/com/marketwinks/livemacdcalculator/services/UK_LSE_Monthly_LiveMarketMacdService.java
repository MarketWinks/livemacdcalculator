...
@RestController
@RequestMapping("/uk_lse_monthly_livemarketmacd")
public class UK_LSE_Monthly_LiveMarketMacdService {
	...
	public boolean UK_LSE_Monthly_LiveMarketMacdParser(@PathVariable String symbol) {
		System.gc();
...
//		List<uk_lse_monthly_livemarketmacd> MarketFeeds_full = UK_LSE_Monthly_LiveMarketMacdRepository.findAll();
		...
		List<uk_lse_monthly_livemarketmacd> MarketFeeds_full = mongoTemplate.find(query, uk_lse_monthly_livemarketmacd.class);
		...
		try {
			...
			// List<uk_lse_monthly_livemarketmacd> MarketFeeds_full =
			// UK_LSE_Monthly_LiveMarketMacdRepository.findAll();

			List<uk_lse_monthly_livemarketmacd> MarketFeeds = new ArrayList<uk_lse_monthly_livemarketmacd>();
			...
			for (int i = 0; i < MarketFeedsSizeForSymbol; i++) {
				...
				// MarketFeeds_full = UK_LSE_Monthly_LiveMarketMacdRepository.findAll();
				MarketFeeds.clear();
				...
				Collections.sort(MarketFeeds, new SortbyLatestTime_monthly());
				...
				for (int index = 0; index < MarketFeedsSizeForSymbol; index++) {
					if (MarketFeeds.get(index).getEma12().toString().equals("calculating")
						...
						uk_lse_monthly_livemarketmacd MarketFeed = new uk_lse_monthly_livemarketmacd();
						...
						uk_lse_monthly_livemarketmacd result = UK_LSE_Monthly_LiveMarketMacdRepository.save(MarketFeed);
						...
						UK_LSE_Monthly_LiveMarketMacdRepository.deleteById(MarketFeeds.get(calcStartindex).get_id());
			...
			List<uk_lse_monthly_livemarketmacd> MarketFeedsForMacdJson = MarketFeeds_full.stream()
					.filter(a -> a.getSymbol().equals(symbol)).collect(Collectors.toList());

			Collections.sort(MarketFeedsForMacdJson, new SortbyLatestTime_monthly());
			...
			uk_lse_monthly_livemarketmacdjson uk_lse_monthly_macdjson = new uk_lse_monthly_livemarketmacdjson();
			uk_lse_monthly_macdjson.setMacdjsonref("uk_lse_monthly_macdjson_" + symbol);
			uk_lse_monthly_macdjson.setMacdjson(macdDataforSaving);
			...
			UK_LSE_Monthly_LiveMarketMacdjsonRepository.deleteById(idofdocext);
			// } else {
	...
	// "/UK_LSE_Monthly_LiveMarketMacdForAnyTime/update/{symbol}/{price}/{ema12}/{ema26}/{signal}/{macd}/{histogram}",
	...
	//
	// // save to the db
	// uk_lse_monthly_livemarketmacd MarketFeed = new
	// uk_lse_monthly_livemarketmacd();
	// MarketFeed.setPrice(price);
	// MarketFeed.setSymbol(symbol);
	...
	// MarketFeed.setHistogram(histogram);
	//
	// uk_lse_monthly_livemarketmacd result =
	// UK_LSE_Monthly_LiveMarketMacdRepository.save(MarketFeed);
	// execution_result = true;
	// return execution_result;
	//
	// }

	@RequestMapping(value = "/UK_LSE_Monthly_LiveMarketMacdUpdateByObjectId/update/{objectId}/{ema12}/{ema26}/{signal}/{macd}/{histogram}", method = RequestMethod.GET)
	public boolean LiveMarketMacdForAnyTimeParser(@PathVariable String objectId, @PathVariable String ema12,
		...
		for (int u = 0; u < MarketFeeds_full.size(); u++) {
			...
			if (MarketFeeds_full.get(u).get_id().equals(objectId)) {
				...
				// attempt to delete
				UK_LSE_Monthly_LiveMarketMacdRepository.deleteById(objectId);
				System.out.println("Duplicate record delete attempted");
