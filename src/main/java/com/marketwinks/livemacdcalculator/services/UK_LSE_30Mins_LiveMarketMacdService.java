package com.marketwinks.livemacdcalculator.services;
...
import com.marketwinks.livemacdcalculator.model.uk_lse_30mins_livemarketmacdjson;
...
@RestController
@RequestMapping("/uk_lse_30mins_livemarketmacd")
public class UK_LSE_30Mins_LiveMarketMacdService {

	@Autowired
	private UK_LSE_30Mins_LiveMarketMacdRepository UK_LSE_30Mins_LiveMarketMacdRepository;

	@Autowired
	private UK_LSE_30Mins_LiveMarketMacdjsonRepository UK_LSE_30Mins_LiveMarketMacdjsonRepository;
	...
	public boolean UK_LSE_30Mins_LiveMarketMacdParser(@PathVariable String symbol) {
		System.gc();
		boolean execution_result = false;
		int calcStartindex = 0;
		int MarketFeedsSizeForSymbol = 0;

//		List<uk_lse_30mins_livemarketmacd> MarketFeeds_full = UK_LSE_30Mins_LiveMarketMacdRepository.findAll();
//		
		...
		List<uk_lse_30mins_livemarketmacd> MarketFeeds_full = mongoTemplate.find(query, uk_lse_30mins_livemarketmacd.class);

		try {

			System.out.println("MACD Calculation started for:" + symbol);

			// List<uk_lse_30mins_livemarketmacd> MarketFeeds_full =
			// UK_LSE_30Mins_LiveMarketMacdRepository.findAll();

			List<uk_lse_30mins_livemarketmacd> MarketFeeds = new ArrayList<uk_lse_30mins_livemarketmacd>();
			...
			for (int i = 0; i < MarketFeedsSizeForSymbol; i++) {
				...
				// MarketFeeds_full = UK_LSE_30Mins_LiveMarketMacdRepository.findAll();
				MarketFeeds.clear();
				...
				Collections.sort(MarketFeeds, new SortbyLatestTime_30min());
				...
				for (int index = 0; index < MarketFeedsSizeForSymbol; index++) {
					if (MarketFeeds.get(index).getEma12().toString().equals("calculating")
						...
						uk_lse_30mins_livemarketmacd MarketFeed = new uk_lse_30mins_livemarketmacd();
						...
						uk_lse_30mins_livemarketmacd result = UK_LSE_30Mins_LiveMarketMacdRepository.save(MarketFeed);
						...
						UK_LSE_30Mins_LiveMarketMacdRepository.deleteById(MarketFeeds.get(calcStartindex).get_id());
			...
			List<uk_lse_30mins_livemarketmacd> MarketFeedsForMacdJson = MarketFeeds_full.stream()
					.filter(a -> a.getSymbol().equals(symbol)).collect(Collectors.toList());

			Collections.sort(MarketFeedsForMacdJson, new SortbyLatestTime_30min());
			...
			uk_lse_30mins_livemarketmacdjson uk_lse_30mins_macdjson = new uk_lse_30mins_livemarketmacdjson();
			uk_lse_30mins_macdjson.setMacdjsonref("uk_lse_30mins_macdjson_" + symbol);
			uk_lse_30mins_macdjson.setMacdjson(macdDataforSaving);
			...
			MongoCollection<org.bson.Document> uk_lse_30mins_livemarketmacdjsonCollection = TestDB
					.getCollection("uk_lse_30mins_livemarketmacdjson");

			// find one document with new Document
			org.bson.Document doc = uk_lse_30mins_livemarketmacdjsonCollection
					.find(new org.bson.Document("macdjsonref", "uk_lse_30mins_macdjson_" + symbol)).first();
			...
			UK_LSE_30Mins_LiveMarketMacdjsonRepository.deleteById(idofdocext);
			// } else {
			...
			uk_lse_30mins_livemarketmacdjson jsonsaveresult = UK_LSE_30Mins_LiveMarketMacdjsonRepository
					.save(uk_lse_30mins_macdjson);
			// uk_lse_30mins_macdjson_<symbol> --> macdDataforSaving
		//	mongoClient.close();
	...
	// "/UK_LSE_30Mins_LiveMarketMacdForAnyTime/update/{symbol}/{price}/{ema12}/{ema26}/{signal}/{macd}/{histogram}",
	...
	// MarketFeed.setHistogram(histogram);
	//
	// uk_lse_30mins_livemarketmacd result =
	// UK_LSE_30Mins_LiveMarketMacdRepository.save(MarketFeed);
	// execution_result = true;
	// return execution_result;
	//
	// }

	@RequestMapping(value = "/UK_LSE_30Mins_LiveMarketMacdUpdateByObjectId/update/{objectId}/{ema12}/{ema26}/{signal}/{macd}/{histogram}", method = RequestMethod.GET)
	public boolean LiveMarketMacdForAnyTimeParser(@PathVariable String objectId, @PathVariable String ema12,
		...
		for (int u = 0; u < MarketFeeds_full.size(); u++) {
			...
			if (MarketFeeds_full.get(u).get_id().equals(objectId)) {
				...
				// attempt to delete
				UK_LSE_30Mins_LiveMarketMacdRepository.deleteById(objectId);
				System.out.println("Duplicate record delete attempted");
...
class SortbyLatestTime_30min implements Comparator<uk_lse_30mins_livemarketmacd> {
	// Used for sorting in ascending order of
	// roll number
	public int compare(uk_lse_30mins_livemarketmacd a, uk_lse_30mins_livemarketmacd b) {
